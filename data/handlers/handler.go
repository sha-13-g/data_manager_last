...
func AddRecharge(w http.ResponseWriter, r *http.Request) {
	...
	site := data.GetSiteById(number.Site_id)
	customer := data.GetCustomerById(site.Customer_id)

	t := time.Now().Unix()
	id := customer.Name[:2] + site.Name[:2] + fmt.Sprint(t)
...
func AddIncident(w http.ResponseWriter, r *http.Request) {
	...
	customerId := r.FormValue("customer-name")
...
func UpdateProject(w http.ResponseWriter, r *http.Request) {
	...
	customer.Contact = r.FormValue("customer-contact")
	customer.Update()
